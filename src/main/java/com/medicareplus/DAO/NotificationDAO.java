...
public class NotificationDAO {
    public boolean add(int userId, String message) throws SQLException, ClassNotFoundException {
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement("INSERT INTO notifications (user_id, message, is_read, created_at) VALUES (?, ?, 0, NOW())")) {
            ps.setInt(1, userId);
    ...
    public List<Notification> findUnreadByUserId(int userId) throws SQLException, ClassNotFoundException {
        ...
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement("SELECT * FROM notifications WHERE user_id=? AND is_read=0 ORDER BY created_at DESC")) {
            ps.setInt(1, userId);
    ...
    public boolean markAsRead(int notificationId) throws SQLException, ClassNotFoundException {
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement("UPDATE notifications SET is_read=1 WHERE notification_id=?")) {
