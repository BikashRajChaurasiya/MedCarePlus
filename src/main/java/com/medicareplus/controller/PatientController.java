...
public class PatientController extends HttpServlet {
	...
	private void showDashboard(HttpServletRequest request, HttpServletResponse response, User user)
		...
		try {
			...
			List<Appointment> a = p == null ? List.of()
					: appointmentService.getAppointmentsByPatientId(p.getPatientId());

			request.setAttribute("patient", p);
			request.setAttribute("totalAppointments", a.size());
			request.setAttribute("pendingCount",
					a.stream().filter(x -> "pending".equalsIgnoreCase(x.getStatus())).count());
			request.setAttribute("approvedCount",
					a.stream().filter(x -> "approved".equalsIgnoreCase(x.getStatus())).count());
			request.setAttribute("completedCount",
					a.stream().filter(x -> "completed".equalsIgnoreCase(x.getStatus())).count());
			request.setAttribute("recentAppointments", a.size() > 5 ? a.subList(0, 5) : a);
			request.setAttribute("recentRecords",
					p == null ? List.of() : appointmentService.getMedicalRecordsByPatientId(p.getPatientId()));
	...
	private void showAppointments(HttpServletRequest request, HttpServletResponse response, User user)
			throws ServletException, IOException {
		try {
			Patient p = patientService.getPatientByUserId(user.getUserId());
			request.setAttribute("appointments",
					p == null ? List.of() : appointmentService.getAppointmentsByPatientId(p.getPatientId()));
	...
	private void showMedicalRecords(HttpServletRequest request, HttpServletResponse response, User user)
			throws ServletException, IOException {
		try {
			Patient p = patientService.getPatientByUserId(user.getUserId());
	...
	private void searchDoctors(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			request.setAttribute("doctors",
					ValidationUtil.hasText(keyword) ? doctorService.getDoctorsBySpecialization(keyword)
	...
	private void bookAppointment(HttpServletRequest request, HttpServletResponse response, User user)
		...
		try {
			...
			Appointment a = new Appointment(p.getPatientId(), Integer.parseInt(request.getParameter("doctorId")), date,
					time, "pending", request.getParameter("symptoms"));
			a.setNotes(request.getParameter("symptoms"));
