...
public class DoctorController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (path == null || "/".equals(path) || "/dashboard".equals(path))
			...
			medicalRecords(request, response, user);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if ("/updateAppointment".equals(path)) {
			...
			saveMedicalRecord(request, user);
			medicalRecords(request, response, user);
		} else if ("/saveMedicalRecord".equals(path)) {
			saveMedicalRecord(request, user);
			medicalRecords(request, response, user);
		} else if ("/contact".equals(path)) {
	...
	private void dashboard(HttpServletRequest request, HttpServletResponse response, User user)
			throws ServletException, IOException {
		try {
			Doctor doctor = doctorService.getDoctorByUserId(user.getUserId());
			java.util.List<com.medicareplus.model.Appointment> appointments = doctor == null
			...
			request.setAttribute("completedCount",
					appointments.stream().filter(a -> "completed".equalsIgnoreCase(a.getStatus())).count());
			request.setAttribute("recentAppointments",
					appointments.size() > 5 ? appointments.subList(0, 5) : appointments);
			request.setAttribute("recentRecords",
					doctor == null ? List.of() : appointmentService.getMedicalRecordsByDoctorId(doctor.getDoctorId()));
	...
	private void medicalRecords(HttpServletRequest request, HttpServletResponse response, User user)
			throws ServletException, IOException {
		try {
			Doctor doctor = doctorService.getDoctorByUserId(user.getUserId());
			request.setAttribute("doctor", doctor);
			request.setAttribute("appointments",
					doctor == null ? List.of() : appointmentService.getAppointmentsByDoctorId(doctor.getDoctorId()));
	...
	private void appointments(HttpServletRequest request, HttpServletResponse response, User user)
			throws ServletException, IOException {
		try {
			Doctor doctor = doctorService.getDoctorByUserId(user.getUserId());
			request.setAttribute("appointments",
	...
	private void updateAppointment(HttpServletRequest request) {
		try {
			appointmentService.updateAppointmentStatus(Integer.parseInt(request.getParameter("appointmentId")),
	...
	private void saveMedicalRecord(HttpServletRequest request, User user) {
		try {
			Doctor doctor = doctorService.getDoctorByUserId(user.getUserId());
			if (doctor == null) {
				throw new SecurityException("Doctor profile not found.");
			}
			...
			record.setSuggestions(request.getParameter("suggestions"));
			record.setTreatmentHistory(request.getParameter("treatmentHistory"));
			record.setNotes(request.getParameter("notes"));
			appointmentService.saveMedicalRecordForDoctor(record, doctor.getDoctorId());
			request.setAttribute("success", "Medical record saved successfully");
		} catch (Exception e) {
			request.setAttribute("error", e.getMessage());
		}
	}
